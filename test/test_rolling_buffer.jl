@testset "RollingBuffer" begin
    ...
    @testset "other constructor" begin
        ...
        @test length(buf) == 0
        @test typeof(buf) <: RollingBuffer{Any}
    end

    @testset "pushfirst" begin
        buf = RollingBuffer{Int}(5)  # New, empty one for full test coverage
        for i in -5:5
            pushfirst!(buf, i)
        end
